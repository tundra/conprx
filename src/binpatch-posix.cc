...
bool PosixPatchEngine::try_open_page_for_writing(address_t addr, dword_t *old_permissions) {
  *old_permissions = 0;
  return try_set_page_permissions(addr, PROT_READ | PROT_WRITE | PROT_EXEC);
}

bool PosixPatchEngine::try_close_page_for_writing(address_t addr, dword_t old_permissions) {
  return try_set_page_permissions(addr, PROT_READ | PROT_EXEC);
}

bool PosixPatchEngine::try_set_page_permissions(address_t fun, int prot) {
  // TODO: deal with the case where the function is spread over multiple pages.
  address_arith_t fun_addr = reinterpret_cast<address_arith_t>(fun);
  address_arith_t page_addr = fun_addr & ~(this->page_size_ - 1);
